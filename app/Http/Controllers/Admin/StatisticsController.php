...
class StatisticsController extends Controller
{
    ...
    */
    ...
    public function getOrder(Request $request)
    {
        ...
        $order_total_done = Orders::where("Status","<>",0)->sum('total');
        ...
        $order_total_date = Orders::where("Status","<>",0)->whereBetween("OrderDate", [$date_from, $date_to])->sum('total');
    ...
    public function getOrderTime(Request $request)
    {
        $list_total = array();
        for ($i = 1; $i < 13; $i++) { 
            $order_time = Orders::where("Status","<>",0)->whereMonth("OrderDate", $i)
                                ->whereYear("OrderDate", now('Asia/Ho_Chi_Minh'))->sum("total");
            array_push($list_total, ['month'=> $i, "order_time" => $order_time]);
        }

        $order_total_year = Orders::where("Status","<>",0)->whereYear("OrderDate", now('Asia/Ho_Chi_Minh'))->sum("total");
        $year = Carbon::now('Asia/Ho_Chi_Minh')->year;
        ...
        if ($request->year) {
            ...
            switch ($year) {
                case '2021':
                    ...
                    $order_total_year = Orders::where("Status","<>",0)->whereYear("OrderDate", $year)->sum("total");
                    break;
                case '2022':
                    $list_total = array();
                    for ($i = 1; $i < 13; $i++) { 
    ...
    {
        $order_total_year = Orders::where("Status","<>",0)->whereMonth("OrderDate", $month)
                             ->whereYear("OrderDate", $year)->sum("total");
